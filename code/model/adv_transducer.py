...
class LangDiscriminator(nn.Module):
    '''
    from https://github.com/ccsasuke/adan/blob/master/code/models.py
    '''
    def __init__(self,
                 hidden_size,
                 num_layers=2,
                 dropout=0.2):
        super(LangDiscriminator, self).__init__()
        assert num_layers >= 0, 'Invalid layer numbers'
        self.net = nn.Sequential()
        for i in range(num_layers):
            if dropout > 0:
                self.net.add_module('lang-dis-dropout-{}'.format(i), nn.Dropout(p=dropout))
            self.net.add_module('lang-dis-linear-{}'.format(i), nn.Linear(hidden_size, hidden_size))
            self.net.add_module('lang-dis-relu-{}'.format(i), nn.ReLU())
        self.net.add_module('lang-dis-linear-final', nn.Linear(hidden_size, 1))

    def forward(self, input):
        '''
        Args:
            input: (batch, seqlen, c_emb_dim)
        '''

        x = torch.mean(input, 1)  # (batch, c_emb_dim)

        return self.net(x)


class AdvTransducer(nn.Module):
    '''
    ...
    def __init__(self, vocab_char, vocab_feat, vocab_pos, 
        ...
        """
        super(AdvTransducer, self).__init__()
        ...
        self.lang_discriminator = LangDiscriminator(2 * self.encoder_hidden_dim)
    ...
    def freeze_transducer(self):
        for p in self.parameters():
            p.requires_grad = False
        for p in self.lang_discriminator.parameters():
    ...
    def clamp_dis(self, clip_lower=-0.01, clip_upper=0.01):
        for p in self.lang_discriminator.parameters():
    ...
    def _dis_loss(self, langs, dis_out):
        vlangs = langs * 2 - 1
        loss = dis_out * vlangs.float()
    ...
    def forward(self, lang, lemma, lemma_len, feat, pos, m_lemma, target=None, 
        ...
        Args:
            ...
            dis_only: only go through the discriminator
        ...
        encode_inpseq, _ = self.encoder(vinpseq)  # (batch, seqlen, num_directions * hidden_size)
        #encode_lemmas = encode_inpseq[:,1:-1,:]
        encode_lemmas = encode_inpseq

        dis_out = self.lang_discriminator(encode_lemmas).squeeze(-1)  # (batch, 1)
        dis_loss = self._dis_loss(lang, dis_out)  
        if dis_only:
            return {'loss': None,
                    'dis_loss': dis_loss,
                    'prediction': None,
